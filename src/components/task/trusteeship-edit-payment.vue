<template>
  ...
  <div class="page-trusteeshipPayment">
    ...
    <div class="detail-list">
      ...
      <div class="detail-item flex">
        <div class="flex cross-center main-between flex1 item-info">
          <div class="item-name">任务保证金
            <!--<span style="color: #999;font-size: 12px;">（已缴纳）</span>-->
          </div>
...
<!--
      ...
      <button class="btn" :class="{ active : ( this.payment[0].WeChat ) }"
              :disabled="!(this.payment[0].WeChat) || !(this.payment[0].WeChat )" @click="doScanCode" v-if="is3thPay" data-tips="扫码支付"> 扫码支付 </button>
-->
    ...
    export default {
        ...
        methods: {
          ...
          doScanQrcodeFailure : function () {
              EventHub.$emit('toastOpen', "即将刷新页面");
              setTimeout(()=>{
                EventHub.$emit('doRefreshRouter', "doRefreshRouter");
              },1500);
          },
          ...
          getTaskInfo : function () {
            if ( sessionStorage.getItem("edit-price") ){
              this.price = sessionStorage.getItem("edit-price");
            }
            if ( sessionStorage.getItem("edit-works") ) {
              this.works = sessionStorage.getItem("edit-works");
            }

            if( parseFloat(this.security_balance) < 1000 ){ //未交保证金
