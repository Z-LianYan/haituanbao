    ...
    <transition name="fade">
      <div class="flex both-center commission-detail" v-if="commissionShow">
        <div class="flex-col commission-box">
          ...
          <div class="tips"> 不含快递面单费用，平台收取3-5元/单的技术服务费，剩下部分以佣金形式结算给买手  </div>
        </div>
      </div>
    </transition>
  ...
  export default {
    ...
    methods: {
      ...
      saveTask : function () {
        ...
        sessionStorage.setItem("price",this.price); //任务佣金
        sessionStorage.setItem("works",this.works); // 任务数量
        this.deposit_balance = parseFloat(sessionStorage.getItem("deposit_balance")); // 账户余额（含1000冻结资金）
        ...
        axios({  // 发布
          ...
          if ( result.code > 0 ) {
            EventHub.$emit('toastOpen', result.message);
            setTimeout(()=>{
              this.$router.push({ name: 'trusteeshipPayment'});
            },2000);
          }
          else if ( result.code === -1 ) {
            EventHub.$emit('toastOpen', result.message);
