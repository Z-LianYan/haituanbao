...
<!--        <div class="list-avatar">
      ...
      <div class="order-box"
        ...
        <div class="flex order-info">
          <div class="item-name">买号</div>
          <div class="item-data">{{item.account}}</div>
        </div>
        ...
        <div class="flex order-info" v-if="nav[1] === 1">
      ...
      <div class="flex action-box">
        ...
        <!-- 待返款（前 已完成） tab-->
        <div class="flex main-end cross-end" v-if="status === 2 && nav[0] === 1" >

          <!-- 这个按钮，是当审核通过的时候，商家先下打款之后，回来点击从而记录已经给用户返款的功能按钮 -->
          <div v-if="item.status === 2 && item.appeal_status === 0" data-tips="已经返款" class="refund primary"
               @click="onConfirmReturn(item.task_id,item.shopping_account_id)">已经返款</div>

          <!-- 驳回返款申诉 按钮 -->
          <div v-if="item.status === 3 && item.appeal_status === 1" data-tips="已再次返款" class="refund primary"
               @click="doAppealReject(item.task_id,item.shopping_account_id)">已再次返款</div>

        </div>

        <!-- 待审核（已返款） tab-->
        <div class="flex main-end cross-end" v-if="status === 5">

          <div v-if="item.appeal_status === 0 " data-tips="已返款" class="refund ">已经返款</div>
          <div v-if="item.appeal_status === -1" data-tips="已再次返款" class="refund ">已经再次返款</div>

          <div data-tips="去审核" class="primary"
               @click="toPageTaskDetail(item.task_id,item.shopping_account_id,item.status,item.level,item.bank_account.realname,item.bank_account.account,item.bank_account.bank,item.bank_account.id)">去审核</div>
        ...
        <div class="flex main-end cross-end" v-if="status === 999">
          ...
          <div v-if="item.status === -6" data-tips="审核不通过" class="refund ">审核不通过</div>

          <!--两个操作按钮-->
          <div v-if="item.status === -6" data-tips="同意"
               class="refund primary" @click="doAppealPass(item.task_id,item.shopping_account_id)">同意</div>
          <div v-if="item.status === -6" data-tips="驳回"
        ...
        <div class="flex main-end cross-end" v-if="status === 3 && nav[4] === 1" >
          ...
          <div v-if="item.status === 6 && item.appeal_status !== 6" data-tips="审核通过" class="refund ">审核通过</div>
          <div v-if="item.status === -6" data-tips="审核不通过" class="refund ">审核不通过</div>

          <div v-if="item.status === 6 && item.appeal_status === 6" data-tips="申诉通过" class="refund ">申诉通过</div>
          <div v-if="item.status === -6 && item.appeal_status === -1" data-tips="申诉驳回" class="refund ">申诉驳回</div>
