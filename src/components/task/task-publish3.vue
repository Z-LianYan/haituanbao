    ...
    <div class="section">
      ...
      <div v-for="(item, index) in rule_item" :key="index" class="flex cross-center admin-item">
        <div class="flex cross-center item-info">
          ...
          <select name="" class="selector" v-model="maihao_requires[index]" @change="onSelected(index)">
 ...
 /*       stime : function () {
    ...
    methods: {
      ...
      getProductObj : function () {
        ...
        devConsole.log(JSON.parse(sessionStorage.getItem("product_photo")));
        ...
        if( this.traffic_type === "3" ){ // traffic_type === 3 的时候传淘口令
          ...
          axios({
            ...
            if ( result.code > 0 ) {
              ...
              this.$router.push({ name : 'taskPublish4' });
            }
      ...
      getTaskTags : function () {
        axios({
          ...
          if ( result.code > 0 ) {
              ...
              let tempTags = [];
            tempTags.length = this.taskTags.length;
